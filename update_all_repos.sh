...
BASEDIR=$(dirname "$0")
YOUTUBE_ADDON=./zips/release/script.module.youtube.dl/addon.xml
...
if test -f "$YOUTUBE_ADDON"; then
    echo "$FILE exists."
    release_youtube_current_version="$(extract_addon_version "$(cat $FILE)")"
    release_youtube_last_version="$(extract_addon_version "$(wget https://raw.github.com/Catch-up-TV-and-More/script.module.youtube.dl/master/script.module.youtube.dl/addon.xml -q -O -)")"
    echo -e "\t* Version of script.module.youtube.dl --> repository: $release_youtube_current_version, master branch: $release_youtube_last_version"
    if [ "$release_youtube_current_version" != "$release_youtube_last_version" ]; then
       need_to_update_repo="no"
    fi
else
    need_to_update_repo="no"
...
if [ "${need_to_update_repo}" == "yes" ]; then
    ...
    python3 ./create_repository.py \
        ...
        https://github.com/Catch-up-TV-and-More/script.module.youtube.dl\#master:script.module.youtube.dl
else
...
## resource.images.catchuptvandmore
beta_images_current_version="$(extract_addon_version "$(cat ./zips/beta/resource.images.catchuptvandmore/addon.xml)")"
beta_images_last_version="$(extract_addon_version "$(wget https://raw.github.com/Catch-up-TV-and-More/resource.images.catchuptvandmore/master/resource.images.catchuptvandmore/addon.xml -q -O -)")"
echo -e "\t* Version of resource.images.catchuptvandmore --> repository: $release_images_current_version, master branch: $release_images_last_version"
if [ "$beta_images_current_version" != "$beta_images_last_version" ]; then
    need_to_update_repo="yes"
fi
...
if [ "${need_to_update_repo}" == "yes" ]; then
    ...
    python3 ./create_repository.py \
        ...
        https://github.com/Catch-up-TV-and-More/resource.images.catchuptvandmore\#master/ \
        https://github.com/Catch-up-TV-and-More/script.module.youtube.dl\#master/
